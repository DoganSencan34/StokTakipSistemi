...
{
    public partial class frmCikisFisi : Form
    {
        public frmCikisFisi()
        {
            InitializeComponent();
        ...
        private void frmCikisFisi_Load(object sender, EventArgs e)
        {
        ...
        private void dataGridView1_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Right)
            {
                ...
                item.Click += new EventHandler(delegate (object s, EventArgs args)
                {
                    ...
                    frmCikisFisiEkle ckisekle = new frmCikisFisiEkle();
                    ckisekle.fisid = fisid;
                ...
                item.Click += new EventHandler(delegate (object s, EventArgs args)
                {
                    ...
                    if (MessageBox.Show("Silmek İstediğinize Eminmisiniz?", "Uyarı", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        ...
                        string adet = "select * from FisHaraket where FisTipi=2 and FisId=" + fisidsil;
                        ...
                        if (kontroltable.Rows.Count > 1)
                        {
                            ...
                            where fb.FisId = fh.FisId and fh.UrunRef = urn.UrunRef and fh.FisTipi = 2";
                            girisfistable = DataBase.select(sorgu);
                            dataGridView1.DataSource = girisfistable;
                            dataGridView1.Columns[0].Visible = false;
                            dataGridView1.Columns[6].Visible = false;
        ...
        {
            frmCikisFisiEkle frmCikisFisiEkle = new frmCikisFisiEkle();
            frmCikisFisiEkle.Show();
            this.Hide();
        }
        string ara;
            ...
            if (textBox1.Text.Trim() != "")
            {
                ara = @"select fb.FisId,fb.FisNo,fb.Tarih,urn.UrunAdi,fh.Miktar,fh.Birim,fh.FisHaraketId 
                                from FisBaslik fb, FisHaraket fh ,Urunler urn 
		                                  ...
		                                  and 
		                                  fh.UrunRef = urn.UrunRef 
		                                  and fh.FisTipi = 2 and Tarih BETWEEN '" + dateTimePicker1.Value.ToString("yyyy-MM-dd") + "' AND '" 
                                          + dateTimePicker2.Value.ToString("yyyy-MM-dd") + "' and fb.FisNo='" + textBox1.Text.Trim() + "'";
            }
            else
            {
                ara = @"select fb.FisId,fb.FisNo,fb.Tarih,urn.UrunAdi,fh.Miktar,fh.Birim,fh.FisHaraketId 
